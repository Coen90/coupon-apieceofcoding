...
part-6/   Redis 대기실: create_big_coupon.sh, waiting_room_flood.js, run.sh, verify.sh
```
...
# part-6 (트래픽 제어)
./scripts/load/part-6/run.sh single   # 서버 1대의 통과 속도
./scripts/load/part-6/run.sh scale    # 서버 2대에서도 전역 통과 속도 유지
./scripts/load/part-6/verify.sh       # FIFO, 순번 유지, 입장권 확인
```
...
6-1 은 Redis 대기실의 FIFO와 ShedLock을 이용한 전역 통과 속도를 검증한다.
