...
GATEWAY_BASE_URL="${GATEWAY_BASE_URL:-http://localhost:8090}"
APPLICATION_BASE_URL="${APPLICATION_BASE_URL:-http://localhost:8080}"
USERS="${USERS:-200}"
...
for _ in $(seq 1 60); do
  if curl -fsS "$GATEWAY_BASE_URL/api/users/me/issuances" -H "X-User-Id: 1" >/dev/null 2>&1; then
    ready=true
    break
  fi
  sleep 1
