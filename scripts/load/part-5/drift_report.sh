...
has_history="$(mysql_scalar "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='coupon' AND table_name='issuance_history'")"
...
printf '  총 수량          : %s\n' "$total"
printf '  DB 발급 수       : %s (ISSUED %s, %s %s)\n' "$issued" "$issued_rows" "$canceled_label" "$canceled_rows"
